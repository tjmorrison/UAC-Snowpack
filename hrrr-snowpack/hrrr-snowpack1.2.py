...
# Station elevation in m (currently Atwater based on mesowest)
# Used for snow level and SLR calculations...critical to use rather
#   than HRRR elevation
siteelev = 2668.0
...
def processhrrr (yr, mn, dy, hr, fhr):
    ...
    hrrrdata = xr.merge([orog_in,u10m_in,v10m_in,t2m_in,rh2m_in, tsfc_in, psfc_in, dswrf_in,uswrf_in,dlwrf_in,ulwrf_in,
                         upress_in,vpress_in,hpress_in,tpress_in,hourly_precip_in], compat='override') 
    ...
    # Load keys, scalar, and model and run for slr
    ...
    nearest_slr = np.round(float(model.predict(data_norm)),1)

    # Adjust SLR if in or below snow level
#    snowlev = slcdf.loc[slcdf.HRRR_Forecast_Hour == int(fhr), 'HRRR_WBZ_mMSL_m'].values[0]  # snow level in meters
#    if snowlev > CLNelev and snowlev < CLNelev+mlthick:
#        altadf.loc[altadf.HRRR_Forecast_Hour == int(fhr), 'CLN_SLR']=altadf.loc[altadf.HRRR_Forecast_Hour == int(fhr), 'CLN_SLR']*(CLNelev+mlthick-snowlev)/float(mlthick)
#    if snowlev > CLNelev+mlthick:
#        altadf.loc[altadf.HRRR_Forecast_Hour == int(fhr), 'CLN_SLR']=0.0

    # If SLR < 3, just make it zero
#    if altadf.loc[altadf.HRRR_Forecast_Hour == int(fhr), 'CLN_SLR'].values[0] < 3.0:
#        altadf.loc[altadf.HRRR_Forecast_Hour == int(fhr), 'CLN_SLR'] = 0.0
